			<div class="leftside">
				...
				if(isset($_GET['ok'])) {
					if(isset($_SESSION['id'])) {
						$query = $login -> prepare('SELECT votes, heurevote, PtsVote FROM accounts WHERE guid = ' . $_SESSION['id'] . ';');
						$query -> execute();
						$row = $query -> fetch();
						$query -> closeCursor();
						...
						if(!($diffDate > 60 * 60 * 3)) {
							...
							$query = $login -> prepare("UPDATE accounts SET votes = " . ($curVotes + 1) . ", heurevote = '" . $curDate . "', PtsVote = " . ($curPoints + PTS_PER_VOTE) . " WHERE guid = " . $_SESSION['id'] . ";");
							$query -> execute();
							$query -> closeCursor();
