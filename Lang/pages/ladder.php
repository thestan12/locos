<?php include('./class/PlayerJobs.class.php'); ?>
			<div class="leftside">
				<ol class="breadcrumb">
						...
						<div id="myTabContent4" class="tab-content padding-20">
							<div role="tabpanel" class="tab-pane fade <?php if(!isset($_POST['ok'])) echo 'active in'; else echo ''; ?>" id="pvm" aria-labelledby="pvm-tab">
								<div class="row">
									<div class="col-md-12">
										<?php				
										$query = $login -> prepare("SELECT p.account, p.level, p.xp, p.class, p.sexe, p.name, p.alignement, a.guid, p.groupe AS gm FROM players AS p , accounts AS a WHERE p.account =  a.guid AND p.groupe < 1 ORDER BY p.xp DESC LIMIT 0, 50");
							...
							<div role="tabpanel" class="tab-pane fade" id="pvp" aria-labelledby="pvp-tab">
								<div class="row">
									<div class="col-md-12">
										<?php				
										$query = $login -> prepare("SELECT p.account, p.level, p.honor, p.class, p.sexe, p.name, p.alignement, a.guid, p.groupe AS gm FROM players AS p , accounts AS a WHERE p.account =  a.guid AND p.groupe < 1 ORDER BY p.honor DESC LIMIT 0, 50");
							...
							<div role="tabpanel" class="tab-pane fade" id="guilds" aria-labelledby="guilds">
								<div class="row">
									<div class="col-md-12">
										<?php				
										$query = $login -> prepare("SELECT name, emblem, lvl, xp  FROM guilds ORDER BY xp DESC LIMIT 0, 50;");
									...
									<div class="col-md-12">
										<form class="form-inline" role="form" method="post" action="#" style="margin-bottom: 10px;">
											<select class="form-control" style="width: 90%!important; height: 35px; padding: 6px 12px!important;" name="job">
												<?php 
												$query = $jiva -> prepare("SELECT id, name, skills FROM jobs_data WHERE  skills != '';");			
												$query -> execute();
												$count = $query -> rowCount();
												$query -> setFetchMode(PDO:: FETCH_OBJ);
										...
										<div class="panel panel-primary">
											...
											<div class="panel-body">
												...
												while($row = $query -> fetch()) {
													echo '<img style="width: 25px; height: 25px; margin-bottom: 1px; margin-left: 1px; border: 1px solid black; border-radius: 7px; -moz-border-radius: 7px; -webkit-border-radius: 7px;" src="' . URL_SITE . 'img/dofus/img/job/' . $row -> id . '.png" data-toggle="tooltip" title="" data-original-title="' . $row -> name . '"/>';
												}
												?>
											</div>
										</div>
										...
										if(isset($_POST['ok']) && isset($_POST['job'])) {
											$query = $login -> prepare("SELECT name, level, jobs FROM players WHERE jobs LIKE '%" . $_POST['job'] . "%' LIMIT 0, 50;");			
											$query -> execute();
											$count = $query -> rowCount();
											$query -> setFetchMode(PDO:: FETCH_OBJ);
												...
												foreach($players as $player) { 
													...
													<tr>
														...
														foreach($player -> otherJobs as $id) {
															echo '<img style="margin-right: 5px;" src="' . URL_SITE . 'img/dofus/img/job/' . $id . '.png"/>';
														}
														?>
														</td>
													</tr>
