			...
			<div class="leftside">
				...
				if($server != -1) { ?>
					<form class="form-inline" role="form" method="post" action="?page=shop&server=<?php echo $server; ?>" style="margin-bottom: 10px;">
						<select class="form-control" style="width: 90%!important; height: 35px; padding: 6px 12px!important;" name="category">
							<?php 
							$query = $connection -> prepare("SELECT id, name FROM `site.shop.categories`;");			
							$query -> execute();
							$count = $query -> rowCount();
							$query -> setFetchMode(PDO:: FETCH_OBJ);

							while($row = $query -> fetch())
								echo '<option value="' . $row -> id . '" ' . (isset($_POST['category']) && $row -> id == $_POST['category']  ? 'selected' : '') . '>' . $row -> name . '</option>';
							$query -> closeCursor();
							?>
						</select>
				...
				<?php 
					if($category != -1) {
						$query = $connection -> prepare("SELECT * FROM `site.shop.objects` WHERE `category` = " . $category . " AND server LIKE '%" . $server . "%';");			
						$query -> execute();
						$count = $query -> rowCount();
						$query -> setFetchMode(PDO:: FETCH_OBJ);
