...
public class GameClient {
    ...
    public void parsePacket(String packet) throws InterruptedException {
        ...
        World.world.logger.info("START EVENT" + packet.charAt(0));
        ...
        World.world.logger.info("STOP EVENT");
    }
    ...
    private void parseAccountPacket(String packet) {
        switch (packet.charAt(1)) {
            ...
            case 'k':
                ...
                break;
            case 'L':
                World.world.logger.info("The player getCharacters");
            ...
            case ExchangeAction.TRADING_WITH_COLLECTOR:
                ...
                for (Player loc : World.world.getGuild(collector.getGuildId()).getMembers()) {
                    if (loc != null && loc.isOnline()) {
                        SocketManager.GAME_SEND_gITM_PACKET(loc, Collector.parseToGuild(loc.get_guild().getId()));
                        String str = "";
    ...
    private void parseGamePacket(String packet) {
        World.world.logger.info("START " + packet.charAt(1));
        switch (packet.charAt(1)) {
            case 'A':
                if (this.player != null)
                    sendActions(packet);
        ...
        World.world.logger.info("STOP " + packet.charAt(1));
    }
    ...
    private void gameParseDeplacementPacket(GameAction GA) {
        ...
        if (this.player.getFight() == null) {
            ...
            if (result == -1000)
                path = CryptManager.getHashedValueByInt(this.player.get_orientation()) + CryptManager.cellID_To_Code(this.player.getCurCell().getId());
            ...
            if (this.player.walkFast) {
                ...
                SocketManager.GAME_SEND_GA_PACKET_TO_MAP(this.player.getCurMap(), "" + GA.id, 1, this.player.getId() + "", "a" + CryptManager.cellID_To_Code(this.player.getCurCell().getId()) + path);
