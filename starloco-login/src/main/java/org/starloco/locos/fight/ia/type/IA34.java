...
public class IA34 extends AbstractNeedSpell  {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            Fighter ennemy0 = Function.getNearestAllnbrcasemax(this.fight, this.fighter, 0, 100);
            if(this.attack >= 1) ennemy0 = Function.getNearestEnnemy(this.fight, this.fighter);

            for(Spell.SortStats spellStats : this.highests)
                if(spellStats != null && spellStats.getMaxPO() > maxPo)
                    maxPo = spellStats.getMaxPO();

            Fighter ennemy1 = Function.getNearestAllnbrcasemax(this.fight, this.fighter, 1, maxPo + 1);// pomax +1;
            Fighter ennemy2 = Function.getNearestAllnbrcasemax(this.fight, this.fighter, 0, 2);//2 = po min 1 + 1;
            ...
            if(this.fighter.getCurPm(this.fight) > 0 && ennemy1 == null && ennemy2 == null) {
                int value = Function.moveautourIfPossible(this.fight, this.fighter, ennemy0);
                if(value != 0) {
                    time = value;
                    action = true;
                    ennemy1 = Function.getNearestAllnbrcasemax(this.fight, this.fighter, 1, maxPo + 1);// pomax +1;
                    ennemy2 = Function.getNearestAllnbrcasemax(this.fight, this.fighter, 0, 2);//2 = po min 1 + 1;
                    if(this.attack >= 1) {
                        ennemy1 = Function.getNearestEnnemynbrcasemax(this.fight, this.fighter, 1, maxPo + 1);// pomax +1;
                        ennemy2 = Function.getNearestEnnemynbrcasemax(this.fight, this.fighter, 0, 2);//2 = po min 1 + 1;
                    }
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && ennemy1 != null && ennemy2 == null && !action) {
                int value = Function.attackAllIfPossible(this.fight, this.fighter, this.highests);
                if(value != 0) {
                    time = value;
                    action = true;
                    this.attack++;
                ...
                int value = Function.attackAllIfPossible(this.fight, this.fighter, this.cacs);
                if(value != 0) {
                    time = value;
                    action = true;
                    this.attack++;
