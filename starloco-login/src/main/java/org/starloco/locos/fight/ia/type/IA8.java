...
public class IA8 extends AbstractNeedSpell  {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            Fighter target = Function.getNearestInvocnbrcasemax(this.fight, this.fighter, 0, maxPo);//2 = po min 1 + 1;
            ...
            if(PM > 0 && target == null) {
                ...
                if(num != 0) {
                    time = num;
                    action = true;
                    target = Function.getNearestInvocnbrcasemax(this.fight, this.fighter, 0, maxPo);//2 = po min 1 + 1;
                }
            }
            ...
            if(PA > 0 && !action && target != null) {
                if (Function.buffIfPossible(this.fight, this.fighter, target, this.buffs)) {
                    time = 400;
                    action = true;
                }
            }
