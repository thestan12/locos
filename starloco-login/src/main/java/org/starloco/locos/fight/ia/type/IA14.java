        ...
        if (this.count > 0 && this.fighter.canPlay() && !this.stop) {
            if (!this.fighter.haveInvocation()) {
                if (!Function.invocIfPossible(this.fight, this.fighter)) {
                    ...
                    int value = Function.moveToAttackIfPossible(this.fight, this.fighter), cellId = value - (value / 1000) * 1000;
                    ...
                    else Function.moveNearIfPossible(fight, this.fighter, target);
                }
            } else {
                Fighter target = Function.getNearestEnnemy(this.fight, this.fighter);
                int value = Function.moveToAttackIfPossible(this.fight, this.fighter), cellId = value - (value / 1000) * 1000;
                ...
                if (this.fight.canCastSpell1(this.fighter, spellStats, this.fighter.getCell(), cellId)) this.fight.tryCastSpell(this.fighter, spellStats, cellId);
                else Function.moveNearIfPossible(this.fight, this.fighter, target);
            }
