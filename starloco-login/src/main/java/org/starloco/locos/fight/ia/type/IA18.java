    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            if(kImpair && dImpair) {
                ...
                int attack = Function.attackIfPossibleDisciplepair(this.fight, this.fighter, kimbo);

                if (attack != 0) {
                    this.fight.getAllGlyphs().stream().filter(glyph -> glyph.getCell().getId() == this.fighter.getCell().getId()).forEach(glyph -> {
                        this.fighter.addBuff(128, 1, 1, 1, true, 3500, "", this.fighter, false);
                ...
                int attack = Function.attackIfPossibleDiscipleimpair(this.fight, this.fighter, kimbo);
