...
public class Fight {
    ...
    public Fight(int type, int id, GameMap map, Player perso, Player init2) {
        ...
        this.start0 = CryptManager.parseStartCell(getMap(), 0);
        this.start1 = CryptManager.parseStartCell(getMap(), 1);
    ...
    public Fight(int id, GameMap map, Player perso, Collector perco) {
        ...
        if (teams.nextBoolean()) {
            ...
            this.start0 = CryptManager.parseStartCell(getMap(), 1);
            this.start1 = CryptManager.parseStartCell(getMap(), 0);
            setSt1(1);
            setSt2(0);
    ...
    public void toggleLockSpec(Player player) {
        if ((this.getInit0() != null && this.getInit0().getId() == player.getId()) || (this.getInit1() != null && this.getInit1().getId() == player.getId())) {
            ...
            if (!this.isViewerOk()) {
                ...
                SocketManager.GAME_SEND_FIGHT_CHANGE_OPTION_PACKET_TO_MAP(player.getCurMap(), '+', 'S', this.id);
                this.getFighters(3).stream().filter(fighter -> fighter.getPersonnage() != null).forEach(fighter -> fighter.getPersonnage().send("Im040"));
            } else {
                SocketManager.GAME_SEND_FIGHT_CHANGE_OPTION_PACKET_TO_MAP(player.getCurMap(), '-', 'S', this.id);
                this.getFighters(3).stream().filter(fighter -> fighter.getPersonnage() != null).forEach(fighter -> fighter.getPersonnage().send("Im039"));
            }
        } else {
            player.send("BN");
        }
    ...
    public synchronized int tryCastSpell(Fighter fighter, SortStats spell, int cell) {
        ...
        if (fighter.getPersonnage() != null) {
            TimerWaiter.addNext(() -> {
                if(!(spell.getSpellID() == 693 || spell.getSpellID() == 696))
                    this.setCurAction("");
    ...
    public boolean onFighterDeplace(Fighter fighter, GameAction GA) {
        ...
        if (!current.isHide()) {
            SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, GA.id, "1", current.getId() + "", "a" + CryptManager.cellID_To_Code(fighter.getCell().getId()) + newPath);
        } else {
            if (current.getPersonnage() != null) {
                // On envoie le path qu'au joueur qui se d�place
                GameClient out = current.getPersonnage().getGameClient();
                SocketManager.GAME_SEND_GA_PACKET(out, GA.id + "", "1", current.getId() + "", "a" + CryptManager.cellID_To_Code(fighter.getCell().getId()) + newPath);
            }
        }
    ...
    public boolean playerReconnect(final Player perso) {
        ...
        if (getState() == Constant.FIGHT_STATE_PLACE) {
            ...
            SocketManager.GAME_SEND_GS_PACKET(perso);// D�but du jeu
    ...
    public String getGE(int win) {
        try {
            ...
            if (mobCapturable && !SoulStone.isInArenaMap(this.getMapOld().getId())) {
                ...
                setFullSoul(new SoulStone(Database.getStatics().getObjectData().getNextId(), 1, 7010, Constant.ITEM_POS_NO_EQUIPED, pierreStats)); // Cr�e la pierre d'�me
                ...
                if (getCapturer().size() > 0 && !SoulStone.isInArenaMap(this.getMapOld().getId())) // S'il y a des captureurs
                {
                    for (int i = 0; i < getCapturer().size(); i++) {
                        try {
                            Fighter f = getCapturer().get(Formulas.getRandomValue(0, getCapturer().size() - 1)); // R�cup�re un captureur au hasard dans la liste
                            if(f != null && f.getPersonnage() != null) {
                                ...
                                Couple<Integer, Integer> pierreJoueur = Formulas.decompPierreAme(f.getPersonnage().getObjetByPos(Constant.ITEM_POS_ARME));// R�cup�re les stats de la pierre �quipp�
