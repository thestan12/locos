...
public class IA46 extends AbstractNeedSpell  {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            Fighter A = Function.getNearestFriend(this.fight, this.fighter);

            for(Spell.SortStats spellStats : this.highests)
                if(spellStats.getMaxPO() > maxPo)
                    maxPo = spellStats.getMaxPO();

            Fighter L = Function.getNearestAminoinvocnbrcasemax(this.fight, this.fighter, 1, maxPo + 1);// pomax +1;
            if(this.fighter.getCurPa(this.fight) > 0 && (L != null || A != null) && !this.boost) {
                if (Function.buffIfPossible(this.fight, this.fighter, (L == null ? A : L), this.buffs)) {
                    time = 1000;
                    action = true;
                    this.boost = true;
                }
            ...
            if(L != null && (L.getPdv() * 100) / L.getPdvMax() > 99)
                    L = Function.getNearestAminoinvocnbrcasemax(this.fight, this.fighter, 1, maxPo);
            if(L != null) if(L.isHide()) L = null;

            if(this.fighter.getCurPm(this.fight) > 0 && L == null) {
                int value = Function.moveautourIfPossible(this.fight, this.fighter, A);
                if(value != 0) {
                    time = value;
                    action = true;
                    L = Function.getNearestAminoinvocnbrcasemax(this.fight, this.fighter, 1, maxPo + 1);// pomax +1;
                    if(maxPo == 1) L = null;
                }
            }
            if(this.fighter.getCurPm(this.fight) > 0 && !action && L != null && !this.heal) {
                ...
                if(value != 0) {
                    time = value;
                    action = true;
                    L = Function.getNearestinvocateurnbrcasemax(this.fight, this.fighter, 1, maxPo + 1);// pomax +1;
                    if(maxPo == 1) L = null;
                }
            }
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && !action) {
                if (Function.HealIfPossible(this.fight, this.fighter, false, 99) != 0) {
                    time = 2000;
                    action = true;
                    this.heal = true;
                }
