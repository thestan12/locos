...
public class IA51 extends AbstractNeedSpell  {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            Fighter A = Function.getNearestAminbrcasemax(this.fight, this.fighter, 0, 5);//2 = po min 1 + 1;
            Fighter A1 = Function.getNearestAminbrcasemax(this.fight, this.fighter, 0, 2);//2 = po min 1 + 1;
            ...
            if(this.fighter.getCurPm(this.fight) > 0 && C == null && A == null) {
                ...
                if(value != 0) {
                    ...
                    A = Function.getNearestAminbrcasemax(this.fight, this.fighter, 0, 5);//2 = po min 1 + 1;
                    A1 = Function.getNearestAminbrcasemax(this.fight, this.fighter, 0, 2);//2 = po min 1 + 1;
                }
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && !action && A1 != null) {
                if (Function.buffIfPossible(this.fight, this.fighter, A1, this.buffs)) {
                    time = 1000;
                    action = true;
                }
            }
            if(this.fighter.getCurPa(this.fight) > 0 && C != null && !action) {
                int value = Function.attackIfPossibleBuveur(this.fight, this.fighter, C);
                if(value != 0) {
                    time = value;
                    action = true;
                }
