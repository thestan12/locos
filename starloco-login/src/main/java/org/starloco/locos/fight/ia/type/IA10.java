...
public class IA10 extends AbstractIA  {
    ...
    public void apply() {
        if (count > 0 && this.fighter.canPlay() && !this.stop) {
            Fighter target = Function.getNearestEnnemy(fight, this.fighter);
            if (target == null) return;

            int value = Function.moveToAttackIfPossible(fight, this.fighter), cellId = value - (value / 1000) * 1000;
            ...
            if (cellId != -1) {
                ...
                if (!Function.HealIfPossible(fight, this.fighter, true))
                    if (!Function.HealIfPossible(fight, this.fighter, false))
                        this.stop = true;
            } else {
                this.stop = true;
            }
