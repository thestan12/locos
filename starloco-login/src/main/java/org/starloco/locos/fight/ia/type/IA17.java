...
public class IA17 extends AbstractNeedSpell  {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            Fighter target = Function.getNearestEnnemynbrcasemax(this.fight, this.fighter, 0, 2);
            ...
            if(this.fighter.getCurPm(this.fight) > 0 && target == null) {
                ...
                if(num != 0) {
                    time = num;
                    action = true;
                    target = Function.getNearestEnnemynbrcasemax(this.fight, this.fighter, 0, 2);
                }
            }

            if(this.fighter.getCurPa(this.fight) > 0 && target == null) {
                int num = Function.attackBondIfPossible(this.fight, this.fighter, ennemy);
                if(num != 0) {
                    time = num;
                    action = true;
                    target = Function.getNearestEnnemynbrcasemax(this.fight, this.fighter, 0, 2);//2 = po min 1 + 1;
                }
            }
